            ...
            @else
                <div class="modal-header justify-content-center border-0 pb-0" id="kt_modal_add_user_header">
                    <div class="text-center">
                        <h1 class="mb-3 modal_title text-capitalize">OB Request Approval</h1>
                    </div>
                </div>
                        ...
                        <div class="d-print-none border border-dashed @if($data['is_approved'] == 1) border-success  @elseif($data['is_approved']==2)border-danger @else border-gray-300 @endif card-rounded h-lg-100 p-9 bg-lighten">
                            <div class="mb-6">
                                ...
                                @elseif($data['is_approved']== 1)
                                    <span class="badge badge-success me-2 cursor-pointer" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top"
                                    title="Officer :{{ $data['approver'] }} <br> Remarks :{{ $data['approver_remarks'] }}">Approved</span>
                                @elseif($data['is_approved'] == 2)
                                    <span class="badge badge-danger cursor-pointer" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top"
                                ...
                                @if(!$isCurrentApprover && $data['is_approved'] == null)
                                    <span class="badge badge-info me-2">Waiting for next approver</span>
                                @endif
                            ...
                            <div class="mb-6">
                                <div class="fw-semibold text-gray-600 fs-5">OB Filing Date: </div>
                                <div class="fw-bold text-gray-800 fs-4">{{ $data['ob_filing_date'] }}</div>
                            </div>
                            <div class="mb-6">
                            ...
                            <div class="mb-6 col-12">
                                <div class="fw-semibold text-gray-600 fs-5">Destination:</div>
                                <div class="fw-bold text-gray-800 fs-4 text-capitalize">
                                    {{ $data['destination'] }}
                                </div>
                            </div>
                            <div class="mb-6 col-12">
                                <div class="fw-semibold text-gray-600 fs-5">Purpose:</div>
                                <div class="fw-bold text-gray-800 fs-4 text-capitalize">
                                    {{ $data['purpose'] }}
                                </div>
                            </div>
                            <div class="m-0">
                                <div class="fw-semibold text-gray-600 fs-5">Duration (Estimated time):</div>
                                <div class="fw-bold fs-4 text-gray-800 d-flex align-items-center">
                                    {{ $data['ob_time_out'] }} to {{ $data['ob_time_in'] }}
                                </div>
                            </div>
                            <!--end::Item-->
                        </div>
