...
export var dtCompanyLocation = function (param) {

    const _page = $('.page-file-maintenance-settings');
    const _table = 'company_location';
    const _tab = $(`.${_table}`);
    ...
    function initTable(){
        ...
        dataTableHelper.initTable(
            'hris/admin/settings/file_maintenance/company_location/dt',
            {
                filter_status:false,
            },
            [
                ...
                {
                    data: "name", name: "name", title: "Company Location",
                    sortable:false,
                    render: function (data, type, row) {
                        return `${data}<span class="text-gray-500 fw-semibold fs-7 d-block text-start ps-0">${row.company_name}</span>`;
        ...
        $(`#${_table}_table`).ready(function() {
            ...
            $(`#${_table}_table`).on('click','.view-details',function(e){
                ...
                let modal_id = '#modal_add_company_location';
                let form = $('#form_add_company_location');

                let formData = new FormData;

                formData.append('id',id);
                _request.post('/hris/admin/settings/file_maintenance/company_location/info',formData)
                .then((res) => {
            ...
            $(`#${_table}_table`).on('click','.delete',function(e){
                ...
                Alert.confirm('question','Delete this record ?',{
                    onConfirm: function() {
                        formData.append('id',id);
                        _request.post('/hris/admin/settings/file_maintenance/company_location/delete',formData)
                        .then((res) => {
