...
class Position extends Controller
{
    public function dt(Request $rq)
    {
        ...
        $data = HrisPosition::where([['is_deleted',null],['is_active','!=',0]])->orderBy('id', 'ASC') ->get();
    ...
    public function update(Request $rq)
    {
        try{
            ...
            $query = HrisPosition::updateOrCreate($attribute,$values);
    ...
    public function validate_request(Request $rq)
    {
        try{
            ...
            $exists = HrisPosition::where('name', ucwords(strtolower($rq->name)))
            ->when($excluded_id, function ($q) use ($excluded_id) {
                $q->where('id', '!=', $excluded_id);
            })
            ->where('is_active','!=',0)
            ->exists();
            return response()->json(['valid' => !$exists, 'message' => 'The position name is already in use']);
        }catch(Exception $e)
        {
            return response()->json(['status'=>400,'message' =>$e->getMessage()]);
        }
    ...
    public function delete(Request $rq)
    {
        try{
            ...
            return response()->json([
                ...
                'payload' => HrisPosition::where('is_active',1)->count()
            ]);
        }catch(Exception $e){
            DB::rollback();
            return response()->json([
