...
class EmploymentType extends Controller
{
    public function dt(Request $rq)
    {
        ...
        $data = HrisEmploymentType::where([['is_deleted',null],['is_active','!=',0]])->orderBy('id', 'ASC') ->get();
    ...
    public function info(Request $rq)
    {
        try{
            ...
            $query = HrisEmploymentType::find($id);
            $payload = [
                'name' =>$query->name,
    ...
    public function update(Request $rq)
    {
        try{
            ...
            $query = HrisEmploymentType::updateOrCreate($attribute,$values);
    ...
    public function validate_request(Request $rq)
    {
        try{
            ...
            $exists = HrisEmploymentType::where('name', ucwords(strtolower($rq->name)))
            ->when($excluded_id, function ($q) use ($excluded_id) {
                $q->where('id', '!=', $excluded_id);
            })
            ->where('is_active','!=',0)
            ->exists();
            return response()->json(['valid' => !$exists, 'message' => 'This employment type is already in use']);
        }catch(Exception $e)
        {
            return response()->json(['status'=>400,'message' =>$e->getMessage()]);
        }
    ...
    public function delete(Request $rq)
    {
        try{
            ...
            return response()->json([
                ...
                'payload' => HrisEmploymentType::where('is_active',1)->count()
            ]);
        }catch(Exception $e){
            DB::rollback();
            return response()->json([
