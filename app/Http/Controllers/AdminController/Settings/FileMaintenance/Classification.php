...
class Classification extends Controller
{
    public function dt(Request $rq)
    {
        ...
        $data = HrisClassification::where([['is_deleted',null],['is_active','!=',0]])->orderBy('id', 'ASC') ->get();
    ...
    public function info(Request $rq)
    {
        try{
            ...
            $query = HrisClassification::find($id);
            $payload = [
                'name' =>$query->name,
    ...
    public function update(Request $rq)
    {
        try{
            ...
            $query = HrisClassification::updateOrCreate($attribute,$values);
    ...
    public function validate_request(Request $rq)
    {
        try{
            ...
            $exists = HrisClassification::where('name', ucwords(strtolower($rq->name)))
    ...
    public function delete(Request $rq)
    {
        try{
            ...
            return response()->json([
                ...
                'payload' => HrisClassification::where('is_active',1)->count()
            ]);
        }catch(Exception $e){
            DB::rollback();
            return response()->json([
